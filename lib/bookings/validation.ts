...
export type ValidBookingInput = { startTime: string; endTime: string; sampleName: string; sampleCount: number; purpose: string; notes: string | null };
export function validateBookingInput(input: BookingInput): { value: ValidBookingInput | null; fieldErrors: FieldErrors } {
  const fieldErrors: FieldErrors = {}; const start = new Date(input.startTime); const end = new Date(input.endTime); const sampleName = input.sampleName.trim(); const sampleCount = Number(input.sampleCount); const purpose = input.purpose.trim(); const notes = input.notes.trim();
  ...
  if (!Number.isInteger(sampleCount) || sampleCount < 1 || sampleCount > 1000000) fieldErrors.sampleCount = "Sample count must be a whole number between 1 and 1,000,000.";
  ...
  return Object.keys(fieldErrors).length ? { value: null, fieldErrors } : { value: { startTime: start.toISOString(), endTime: end.toISOString(), sampleName, sampleCount, purpose, notes: notes || null }, fieldErrors };
}
