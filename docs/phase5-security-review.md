...
- **Bookings/privacy:** ownership derives from `auth.uid()`. Full rows are owner/admin-only. Availability returns booking ID, instrument ID, start/end, and status only.
