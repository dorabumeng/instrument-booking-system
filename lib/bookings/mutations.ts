...
export async function createBooking(instrumentId: string, input: BookingInput): Promise<ActionResult<Booking>> {
  ...
  const instrument = await auth.supabase.from("instruments").select("id,status,booking_slot_minutes,min_booking_minutes,max_booking_minutes").eq("id", instrumentId).maybeSingle();
  ...
  const value = validated.value; const ruleErrors = validateBookingRules(value.startTime, value.endTime, instrument.data, getLabTimezone()); if (Object.keys(ruleErrors).length) return { success: false, code: "VALIDATION_ERROR", message: "Booking times do not follow this instrument's rules.", fieldErrors: ruleErrors }; const { data, error } = await auth.supabase.from("bookings").insert({ instrument_id: instrumentId, user_id: auth.userId, start_time: value.startTime, end_time: value.endTime, sample_name: value.sampleName, sample_count: value.sampleCount, purpose: value.purpose, notes: value.notes, status: "confirmed" }).select().single();
...
export async function updateBooking(bookingId: string, input: BookingInput): Promise<ActionResult<Booking>> {
  const auth = await authenticatedContext(); if (!auth) return { success: false, code: "UNAUTHENTICATED", message: "Your session has expired. Please sign in again." };
  const validated = validateBookingInput(input); if (!validated.value) return { success: false, code: "VALIDATION_ERROR", message: "Check the highlighted fields.", fieldErrors: validated.fieldErrors };
  const existing = await auth.supabase.from("bookings").select("*, instrument:instruments(booking_slot_minutes,min_booking_minutes,max_booking_minutes)").eq("id", bookingId).eq("user_id", auth.userId).maybeSingle();
  ...
  const value = validated.value; const ruleErrors = validateBookingRules(value.startTime, value.endTime, existing.data.instrument, getLabTimezone()); if (Object.keys(ruleErrors).length) return { success: false, code: "VALIDATION_ERROR", message: "Booking times do not follow this instrument's rules.", fieldErrors: ruleErrors }; const { data, error } = await auth.supabase.from("bookings").update({ start_time: value.startTime, end_time: value.endTime, sample_name: value.sampleName, sample_count: value.sampleCount, purpose: value.purpose, notes: value.notes }).eq("id", bookingId).eq("user_id", auth.userId).select().single();
