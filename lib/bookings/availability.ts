...
export async function loadAvailability(instrumentId: string, rangeStart: Date, rangeEnd: Date): Promise<AvailabilityLoad> {
  ...
  const events: EventInput[] = availability.data.map(slot => { const booking = ownById.get(slot.booking_id); if (booking) { ownById.delete(slot.booking_id); return { id: booking.id, title: booking.sample_name, start: booking.start_time, end: booking.end_time, backgroundColor: "#087f75", borderColor: "#06665e", extendedProps: { kind: "own", booking } }; } return { id: `occupied-${slot.booking_id}`, title: "Occupied", start: slot.start_time, end: slot.end_time, backgroundColor: "#64748b", borderColor: "#475569", extendedProps: { kind: "occupied" } }; });
